...
The RPM spec file provided here is for aethercoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://aethercoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[aethercoin git repository](https://github.com/aethercoin/aethercoin) but are not part of
...
Currently only `Source10` is used, for the example `aethercoin.conf` file.
...
in the aethercoin git repository. At present only an SVG version of the aethercoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the aethercoin
...
needed when 0.12.1 is released, a proper fix is already in the aethercoin git
master branch.
...
idea to build aethercoin Core against a static version of that library compiled
...
The `aethercoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `aethercoin-server` package. At this point, most RPM based
...
The `aethercoin-server` package should have SELinux support. How to properly do
...
aethercoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/aethercoin/). He has been
packaging aethercoin for Fedora at least since 2012.
...
4. I renamed the `aethercoin` package that contains the Qt GUI to `aether` as
that appears to be how the general population refers to it, in contrast to
`aethercoin-xt` or `aethercoin-classic`. I wanted to make sure the general population
