...
###   This script attempts to download the signature file SHA256SUMS.asc from aethercoin.org
...
WORKINGDIR="/tmp/aethercoin"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://aethercoin.org/bin/"
VERSIONPREFIX="aether-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'aethercoin-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the aethercoin Core binary release signing key installed?"
   fi
