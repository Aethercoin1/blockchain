...
const char * const AETHERCOIN_CONF_FILENAME = "aethercoin.conf";
const char * const AETHERCOIN_PID_FILENAME = "aethercoin.pid";
...
#else
    const char* pszModule = "aethercoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\AetherCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\AetherCoin
    // Mac: ~/Library/Application Support/AetherCoin
    // Unix: ~/.aethercoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "AetherCoin4";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/AetherCoin4";
#else
    // Unix
    return pathRet / ".aethercoin4";
#endif
#endif
}
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", AETHERCOIN_CONF_FILENAME));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No aethercoin.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override aethercoin.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", AETHERCOIN_PID_FILENAME));
