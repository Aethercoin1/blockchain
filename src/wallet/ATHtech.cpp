...
UniValue ATHtech::CreateAnonTransaction(string address, CAmount nValue, int nTransactions) {
  ...
  if (encryptionSuccess) {
    UniValue addresses = find_value(serverData, "ATH_addresses");
    UniValue addrArray = addresses.get_array();

    UniValue ATHtechData;
    ATHtechData.setObject();
    ATHtechData.pushKV("anondestination", encryptedAddress);
    ATHtechData.pushKV("public_key", pubKey);
    ATHtechData.pushKV("anonaddress", addrArray);
    ATHtechData.pushKV("anonfee", find_value(serverData, "transaction_fee"));
    ATHtechData.pushKV("min_amount", find_value(serverData, "min_amount"));
    return ATHtechData;
  } else {
    throw runtime_error("Unable to send ATHTech transaction, please try again.");
  }

}

vector<anonServer> ATHtech::GetAnonServers() {

  vector<anonServer> returnServers;

  if (vAddedAnonServers.size() < 1 && mapMultiArgs["-addanonserver"].size() < 1) {
      throw runtime_error("You must have at least one ATHTech server added to your conf file or by rpc command.");
  ...
  if (returnServers.size() < 1) {
      throw runtime_error("Please, add some valid ATHTech server to send private payments.");
      return returnServers;
  }
...
UniValue ATHtech::FindAnonServer(std::vector<anonServer> anonServers, CAmount nValue, int nTransactions) {
  if (anonServers.size() < 1) {
      throw runtime_error("None of your configured ATHTech nodes are available right now.");
  }
  ...
  if (curl) {
    ...
    curl_easy_cleanup(curl);
    curl_global_cleanup();
    throw runtime_error("CURL uATHailable");
  }
}

UniValue ATHtech::ParseJSONResponse(string readBuffer) {
  try {
      UniValue valRequest;
...
string ATHtech::EncryptAddress(string address, string pubKeyStr, int nTransactions, int nPiece, long nId) {

  address = "{\"n\":\""+address+"\",\"t\":"+std::to_string(GetArg("anon_out_delay",ATHTECH_DEFAULT_OUT_DELAY))+",\"p\":"+std::to_string(nPiece)+",\"o\":"+std::to_string(nTransactions)+",\"u\":"+std::to_string(nId)+"}";
...
int ATHtech::PublicEncrypt(unsigned char* data, int data_len, unsigned char* key, unsigned char* encrypted)
{
...
RSA * ATHtech::CreateRSA(unsigned char * key,int isPublic)
{
    RSA *rsa= NULL;
    BIO *keybio ;
...
bool ATHtech::TestEncryption(string encrypted, UniValue serverData) {
...
UniValue ATHtech::GetServerInfo(std::string server) {
  curl_global_init(CURL_GLOBAL_ALL);
  curl = curl_easy_init();
  string readBuffer;
  ...
  if (curl) {
    ...
    if (res != CURLE_OK) {
      ...
      if (!data_obj.isObject()) {
        ...
        UniValue ATHtechData;
        ATHtechData.setObject();
        ATHtechData.pushKV("min_amount", find_value(data, "min_amount"));
        ATHtechData.pushKV("max_amount", find_value(data, "max_amount"));
        ATHtechData.pushKV("transaction_fee", find_value(data, "transaction_fee"));
        curl_easy_cleanup(curl);
        curl_global_cleanup();
        return ATHtechData;
      }
