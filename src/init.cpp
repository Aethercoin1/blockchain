...
// Copyright (c) 2017 The AetherCoin developers
 ...
 * between aethercoind, and aethercoin-qt and non-server tools.
*/
...
void Shutdown()
{
    ...
    RenameThread("aethercoin-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), AETHERCOIN_CONF_FILENAME));
    if (mode == HMM_AETHERCOIND)
    {
#ifndef WIN32
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), AETHERCOIN_PID_FILENAME));
#endif
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/aethercoindev/aether>";
    const std::string URL_WEBSITE = "<https://aethercoin.org>";
...
void ThreadImport(std::vector<boost::filesystem::path> vImportFiles)
{
    ...
    RenameThread("aethercoin-loadblk");
    CImportingNow imp;
...
/** Sanity checks
 *  Ensure that AetherCoin is running in a usable environment with all
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("AetherCoin version %s\n", FormatFullVersion());
}

/** Initialize aethercoin.
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Make sure only a single AetherCoin process is using the data directory.
    ...
    if(GetBoolArg("-staking", true))
        threadGroup.create_thread(boost::bind(&AetherCoinStaker, boost::cref(chainparams)));
#endif
