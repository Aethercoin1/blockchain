...
// Copyright (c) 2017 The AetherCoin Core developers
...
#if defined(NDEBUG)
# error "AetherCoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "AetherCoin Signed Message:\n";
...
bool CheckTxInputs(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight)
{
        ...
        if (!inputs.HaveInputs(tx))
            return state.Invalid(false, 0, "", "Inputs uATHailable");
        CAmount nValueIn = 0;
        CAmount nFees = 0;
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
...
void ThreadScriptCheck() {
    RenameThread("aethercoin-scriptch");
...
//
// Commented - AetherCoin uses now version control
//
        ...
        for (unsigned int i = 0; i < block.vtx[0].vout.size(); i++) {
            ...
            if(block.vtx[0].vout[i].nValue > 0) {
                ...
                if(metadata[nPaymentRequestsCount].isStr()) {
                    ...
                    if(block.vtx[0].vout[i].nValue != prequest.nAmount || prequest.fState != CFund::ACCEPTED || parent.Address != CAetherCoinAddress(address).ToString())
...
static bool AcceptBlock(const CBlock& block, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested, const CDiskBlockPos* dbp, bool* fNewBlock)
{
    ...
    {
    	LogPrintf("Returning from  AcceptBlock() at line 4598");
        return false;
    }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams)
{
    ...
    if (pfrom->nVersion != 0)
    {
        ...
        {
            reason = "You are using an old version of AetherCoin, please update.";
            fObsolete = true;
        }

        if(pfrom->nVersion < 70017 && IsWitnessEnabled(chainActive.Tip(), Params().GetConsensus()))
        {
            reason = "Segregated Witness has been enabled and you are using an old version of AetherCoin, please update.";
            fObsolete = true;
        }

        if(pfrom->nVersion < 70020 && IsCommunityFundEnabled(chainActive.Tip(), Params().GetConsensus()))
        {
            reason = "Community Fund has been enabled and you are using an old version of AetherCoin, please update.";
            fObsolete = true;
        }
