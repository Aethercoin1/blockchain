...
#include "config/aethercoin-config.h"
#endif

#include "main.h"
#include "aethercoingui.h"
#include "aethercoinunits.h"
...
const std::string AetherCoinGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString AetherCoinGUI::DEFAULT_WALLET = "~Default";

AetherCoinGUI::AetherCoinGUI(const PlatformStyle *platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    clientModel(0),
    walletFrame(0),
    ...
    platformStyle(platformStyle)
{
    ...
    {
        progressBar->setStyleSheet("QProgressBar { background-color: #e8e8e8; border: 0px solid grey; border-radius: 10px; padding: 1px; text-align: center; } QProgressBar::chunk { background: QLinearGradient(x1: 0, y1: 0, x2: 1, y2: 0, stop: 0 #0003c7, stop: 1 blue); border-radius: 10px; margin: 0px; }");
    }
...
void AetherCoinGUI::createActions()
{
    QActionGroup *tabGroup = new QActionGroup(this);

    overviewAction = new QAction(platformStyle->SingleColorIcon(":/icons/aethercoin"), tr("&Overview"), this);
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a AetherCoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and aethercoin: URIs)"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your AetherCoin addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified AetherCoin addresses"));
    ...
    openAction->setStatusTip(tr("Open a aethercoin: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible AetherCoin command-line options").arg(tr(PACKAGE_NAME)));
...
void AetherCoinGUI::createMenuBar()
{
#ifdef Q_OS_MAC
    ...
    if(walletFrame)
    {
        ...
        Q_FOREACH(AetherCoinUnits::Unit u, AetherCoinUnits::availableUnits())
        {
            QAction *menuAction = new QAction(QString(AetherCoinUnits::name(u)), this);
...
void AetherCoinGUI::onCurrencySelection(QAction* action)
{
    if (action)
    {
...
void AetherCoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        // toolbar->setStyleSheet(
         //"QPushButton { color: #FFFFFF; font-weight:bold; margin: 0; padding: 0; background-color: #43b5eb; border: 0px; } ";
         //"QPushButton:hover { background-color: #71c7f0; border-bottom: 4px solid #43b5eb; border-left: none; } ";
         //"QPushButton:checked { background-color: #997cc5; border-bottom: 4px solid #7d59b5; border-left: none; } ";
         //"QPushButton:pressed { background-color: #997cc5; border-bottom: 4px solid #7d59b5; border-left: none; } ";
        // "#tabs { color: #ffffff; background-image: url(:/images/background) }");

    	QToolBar* topMenuLogo = new QToolBar(walletFrame->topMenu);
        topMenuLogo->setFixedSize(187,60);
        topMenuLogo->setObjectName("ATHLogo");
        topMenuLogo->move(15,15);
        topMenuLogo->setStyleSheet(
           "#ATHLogo { border-image: url(:/icons/menu_logo)  0 0 0 0 stretch stretch; border: 0px; }");

        topMenu1 = new QPushButton(walletFrame->topMenu);
        topMenu1->setFixedSize(100,30);
        topMenu1->setObjectName("topMenu1");
        connect(topMenu1, SIGNAL(clicked()), this, SLOT(gotoOverviewPage()));
        topMenu1->move(237,35);
        ...
        // ImageButton* ATHLogo2 = new ImageButton();
        // ATHLogo2->setFixedSize(64,64);
        // ATHLogo2->setObjectName("ATHLogo");
        // ATHLogo2->setPixmap(pixmap);
        //
        // toolbar->setMovable(false);
        // toolbar->setToolButtonStyle(Qt::ToolButtonTextBesideIcon);
        // toolbar->addWidget(ATHLogo);
        // toolbar->addWidget(ATHLogo2);
...
void AetherCoinGUI::setClientModel(ClientModel *clientModel)
{
...
#ifdef ENABLE_WALLET
bool AetherCoinGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
void AetherCoinGUI::startVotingCounter()
{
    if (GetBoolArg("-staking", true))
    {
...
bool AetherCoinGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void AetherCoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void AetherCoinGUI::repairWallet()
{
...
void AetherCoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void AetherCoinGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void AetherCoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void AetherCoinGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void AetherCoinGUI::optionsClicked()
{
...
void AetherCoinGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void AetherCoinGUI::showDebugWindow()
{
...
void AetherCoinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void AetherCoinGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void AetherCoinGUI::openClicked()
{
...
void AetherCoinGUI::gotoOverviewPage()
{
...
void AetherCoinGUI::gotoHistoryPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void AetherCoinGUI::gotoReceiveCoinsPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void AetherCoinGUI::gotoRequestPaymentPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void AetherCoinGUI::gotoSendCoinsPage(QString addr)
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void AetherCoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void AetherCoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void AetherCoinGUI::setNumConnections(int count)
{
    QString icon;
    switch(count)
    {
    ...
    labelConnectionsIcon->setPixmap(platformStyle->SingleColorIcon(icon).pixmap(0,0));
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to AetherCoin network", "", count));
    if(walletFrame){
...
void AetherCoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if(!clientModel)
        return;
    ...
    if(secs < 90*60)
    {
        ...
        progressBar->setVisible(false);
        if(walletFrame)
            walletFrame->setStatusTitle(tr("Connected to AetherCoin network."));
    }
    else
    {
        ...
        if(walletFrame)
            walletFrame->setStatusTitle(tr("Connecting to AetherCoin network..."));
...
void AetherCoinGUI::showVotingDialog()
{
...
void AetherCoinGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("AetherCoin"); // default title
...
void AetherCoinGUI::changeEvent(QEvent *e)
{
...
void AetherCoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void AetherCoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void AetherCoinGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(AetherCoinUnits::formatWithUnit(unit, amount, true)) +
                  tr("Type: %1\n").arg(type);
    if (!label.isEmpty())
        msg += tr("Label: %1\n").arg(label);
...
void AetherCoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void AetherCoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool AetherCoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool AetherCoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void AetherCoinGUI::setEncryptionStatus(int status)
{
...
void AetherCoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void AetherCoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void AetherCoinGUI::detectShutdown()
{
...
void AetherCoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void AetherCoinGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
static bool ThreadSafeMessageBox(AetherCoinGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void AetherCoinGUI::subscribeToCoreSignals()
{
...
void AetherCoinGUI::unsubscribeFromCoreSignals()
{
    ...
    menu(0)
{
    ...
    QList<AetherCoinUnits::Unit> units = AetherCoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    Q_FOREACH (const AetherCoinUnits::Unit unit, units)
    {
        max_width = qMax(max_width, fm.width(AetherCoinUnits::name(unit)));
    }
    setMinimumSize(max_width, 0);
    setAlignment(Qt::AlignRight | Qt::AlignVCenter);
    setStyleSheet(QString("QLabel { color : blue }"));//.arg(platformStyle->SingleColor().name()));
}
...
{
    setText(AetherCoinUnits::name(newUnits));
}
...
void AetherCoinGUI::toggleStaking()
{
...
void AetherCoinGUI::updateWeight()
{
    if (!pwalletMain)
        return;
...
void AetherCoinGUI::updatePrice()
{
  ...
  request.setUrl(QUrl("https://api.coinmarketcap.com/v1/ticker/ATH-coin/?convert=EUR"));
...
void AetherCoinGUI::replyFinished(QNetworkReply *reply)
{
...
void AetherCoinGUI::replyVotingFinished(QNetworkReply *reply)
{
  ...
  CAetherCoinAddress addr("NMYuCvBiRgvkzjdEBGJHj7rpAnRmfUD6gw");
  CKeyID keyID;
  addr.GetKeyID(keyID);
...
void AetherCoinGUI::getVotingInfo()
{
    QNetworkAccessManager *manager = new QNetworkAccessManager();
    QNetworkRequest request;
    QNetworkReply *reply = NULL;
    ...
    config.setProtocol(QSsl::TlsV1_2);
    request.setSslConfiguration(config);
    request.setUrl(QUrl(QString("https://www.aethercoin.org/voting.") + QString((GetBoolArg("-testnet",false) ? "testnet." : "mainnet.")) + QString("json")));
    request.setHeader(QNetworkRequest::ServerHeader, "application/json");
    reply = manager->get(request);
    connect(manager, SIGNAL(finished(QNetworkReply*)), this,
...
void AetherCoinGUI::updateStakingStatus()
{
    updateWeight();
