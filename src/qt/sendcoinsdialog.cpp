    ...
    platformStyle(platformStyle)
{
   ...
   // connect(ui->notcntechButton, SIGNAL(clicked()), this, SLOT(showtcnTechDialog()));
...
void SendCoinsDialog::checkATHtechServers()
{
    bool notEnoughServers = vAddedAnonServers.size() < 1 && mapMultiArgs["-addanonserver"].size() < 1;

    //ui->notcntechLabel->setVisible(notEnoughServers);
...
void SendCoinsDialog::on_sendButton_clicked()
{
        ...
        if(entry->validate())
        {
            ...
            if(ui->anonsendCheckbox->checkState() != 0) {
                try
                {
                    ...
                    UniValue ATHtechData = ATHtech.CreateAnonTransaction(recipient.address.toStdString() , recipient.amount / (nTransactions * 2), nTransactions);
                    ...
                    if(serverATHAddresses.size() != nTransactions)
                    {
                        QMessageBox::warning(this, tr("Private payment"),
                                         "<qt>" +
                                         tr("ATHTech server returned a different number of addresses.")+"</qt>");
                        valid = false;
                    }
                    ...
                    if(valid)
                    {
                      ...
                      for(unsigned int i = 0; i < serverATHAddresses.size(); i++)
                      {
                          ...
                          cRecipient.destaddress = QString::fromStdString(serverATHAddresses[i].get_str());
                          ...
                          cRecipient.anondestination = QString::fromStdString(ATHtech.EncryptAddress(recipient.address.toStdString(), pubKey.get_str(), nTransactions, i+(i==serverATHAddresses.size()?0:1), nId));
                          if(!find_value(ATHtechData, "anonfee").isNull()){
                              cRecipient.anonfee = nAmountRound * ((float)find_value(ATHtechData, "anonfee").get_real() / 100.0);
                              cRecipient.transaction_fee = find_value(ATHtechData, "anonfee").get_real();
                          }else
                              valid = false;
    ...
    processSendCoinsReturn(prepareStatus,
        AetherCoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), currentTransaction.getTransactionFee()));
    ...
    const SendCoinsRecipient &rcp = currentTransaction.recipients.first();
    {
        ...
        QString amount = "<b>" + AetherCoinUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), nTotalAmount);
        amount.append("</b>");
        ...
        if(txFee + anonfee > 0)
        {
            ...
            questionString.append(AetherCoinUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), txFee + anonfee));
            ...
            if(rcp.fSubtractFeeFromAmount && anonfee > 0)
            {
                ...
                questionString.append(AetherCoinUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), anonfee));
            }

            if(rcp.isanon){
                questionString.append("<br>" + tr("ATHtech server fee:") +QString(" ")+ QString::number(rcp.transaction_fee) + "% "+ tr(rcp.fSubtractFeeFromAmount ? "" : "(already included)") + "<br>");
                if(rcp.fSubtractFeeFromAmount)
                    questionString.append("<span style='color:#aa0000;'>" + AetherCoinUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), nTotalAmount * ((rcp.transaction_fee/100))) + "</span> " + tr("will be deducted as ATHtech fee.") + "<br>");
    ...
    Q_FOREACH(AetherCoinUnits::Unit u, AetherCoinUnits::availableUnits())
    {
        if(u != model->getOptionsModel()->getDisplayUnit())
            alternativeUnits.append(AetherCoinUnits::formatHtmlWithUnit(u, totalAmount));
    }
    questionString.append(tr("Total Amount %1")
        .arg(AetherCoinUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), totalAmount)));
...
void SendCoinsDialog::showATHTechDialog()
{
    ATHtechsetup* setupATHTech = new ATHtechsetup();
    setupATHTech->setWindowIcon(QIcon(":icons/aethercoin"));
    setupATHTech->setStyleSheet(Skinize());
                                 ...
                                 const CAmount& watchBalance, const CAmount& watchUnconfirmedBalance, const CAmount& watchImmatureBalance)
{
    ...
    if(model && model->getOptionsModel())
    {
        ui->labelBalance->setText(AetherCoinUnits::formatWithUnit(0, balance) + (model->getOptionsModel()->getDisplayUnit() != 0 ?( " (" + AetherCoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), balance) + ")") : ""));
    }
}
...
void SendCoinsDialog::processSendCoinsReturn(const WalletModel::SendCoinsReturn &sendCoinsReturn, const QString &msgArg)
{
    ...
    case WalletModel::AbsurdFee:
        msgParams.first = tr("A fee higher than %1 is considered an absurdly high fee.").arg(AetherCoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), maxTxFee));
        break;
...
void SendCoinsDialog::minimizeFeeSection(bool fMinimize)
{
    ...
    //ui->noATHtechButton  ->setVisible(fMinimize);
    //ui->horizontalLayoutSmartFee->setContentsMargins(0, (fMinimize ? 0 : 6), 0, 0);

    if(fMinimize)
        checkATHtechServers();
    else
       // ui->noATHtechLabel  ->setVisible(false);
...
void SendCoinsDialog::updateFeeMinimizedLabel()
{
  ...
  //  else {
  //      ui->labelFeeMinimized->setText(AetherCoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), ui->customFee->value()) +
...
void SendCoinsDialog::updateMinFeeLabel()
{
    if (model && model->getOptionsModel())
        ui->checkBoxMinimumFee->setText(tr("Pay only the required fee of %1").arg(
            AetherCoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), CWallet::GetRequiredFee(1000)) + "/kB")
        );
}
...
void SendCoinsDialog::updateSmartFeeLabel()
{
    ...
    {
        ui->labelSmartFee->setText(AetherCoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(),
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        CAetherCoinAddress addr = CAetherCoinAddress(text.toStdString());
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid AetherCoin address"));
        }
        else // Valid address
        {
            CKeyID keyid;
