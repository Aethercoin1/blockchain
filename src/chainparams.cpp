...
{
    const char* pszTimestamp = "A new genesis block";
...
public:
    CMainParams() {
        ...
        consensus.nRuleChangeActivationThreshold = 15120; // 75% of 20160 //75% of 2880
        consensus.nMinerConfirmationWindow = 20160;//Because voting period reduced to 1 day
        consensus.nStakeMinAge = 60 * 60 * 2;	// minimum for coin age: 2 hours //Changed to 1 hour
        ...
        consensus.nLastPOWBlock = 10000;//Reduced to 300 block
        consensus.nVotingPeriod = 2880 * 1; // 7 Days
...
public:
    CTestNetParams() {
        ...
        consensus.nStakeCombineThreshold = 1000 * COIN;
        consensus.nStakeSplitThreshold = 2 * consensus.nStakeCombineThreshold;
        consensus.nDailyBlockCount =  (1 * 60 * 60) / consensus.nTargetSpacing;
        consensus.nModifierInterval = 10 * 60; // time to elapse before new modifier is computed
        consensus.nTargetTimespan = 25 * 30;
...
public:
    CRegTestParams() {
        ...
        genesis = CreateGenesisBlock(1526564385, 2042883868, 0x207fffff, 1, 50 * COIN);
        consensus.hashGenesisBlock = genesis.GetHash();
