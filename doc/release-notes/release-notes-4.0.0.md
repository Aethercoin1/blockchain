...
aethercoin Core now uses 'headers-first synchronization'. This means that we first ask peers for block headers and validate those. In a second stage, when the headers have been discovered, we download the blocks. However, as we already know about the whole chain in advance, the blocks can be downloaded in parallel from all available peers.
...
Blocks without transactions: http://localhost:9443/rest/block/notxdetails/*HASH*.*EXT* Transactions (requires -txindex): http://localhost:9443/rest/tx/*HASH*.*EXT*
...
Starting with Tor version 0.2.7.1 it is possible, through Tor's control socket API, to create and destroy 'ephemeral' hidden services programmatically. aethercoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available), aethercoin Core automatically creates a hidden service to listen on, without manual configuration. aethercoin Core will also use Tor automatically to connect to other .onion nodes if the control socket can be successfully opened. This will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if aethercoin Core is listening, and a connection to Tor can be made. It can be configured with the -listenonion, -torcontrol and -torpassword settings. To show verbose debugging information, pass -debug=tor.
...
There is no distinction between internal (change) and external keys. HD wallets are incompatible with older versions of aethercoin Core. Segregated Witness
...
### New ATHTech RPC Commands

A command to generate a private payment has been added. aethercoin Core will take care of the negotiation with the ATHTech nodes and will create and broadcast the transaction. The syntax is:
...
### Management of ATHTech servers

ATHTech servers added/removed with the RPC command addanonserver are now stored on the config file when specified by an extra parameter.
